...
If the pass updates closeout state such as `handoffs/active/current_handoff.md`, checkpoint docs, or other control-layer records that imply completion, the execution layer must still ensure the required report artifact is written before claiming success.
