...
This file is the portable supervision packet for Harbor Analytics when the supervisor does not have direct repository access.
...
- Project: `Harbor Analytics`
- Purpose: bounded control-layer governance for an existing research repository
- Scope boundary: initialize and supervise the control layer without restructuring the current project architecture
...
- The repo was initialized in `overlay` mode from its own `README.md`. [E]
- `src/`, `data/`, `notebooks/`, and `site/` remained in place. [E]
- `docs/00_project_scope.md`, `docs/08_status_checkpoint.md`, and this brief now describe the target repo rather than the template. [E]
...
- Bootstrap supervision and define the first bounded task for Harbor Analytics. [E]
