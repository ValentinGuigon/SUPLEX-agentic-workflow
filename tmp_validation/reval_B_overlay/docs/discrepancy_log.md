...
| 2026-04-13 | `controlled_target_repo_validation_pass` | Applying the current template by straight control-layer copy leaves repo-state docs such as `docs/00_project_scope.md` and `docs/09_supervision_brief.md` describing `suplex-template` instead of the target repo. | Supervision with repo access becomes inconsistent and the portable supervision packet is not trustworthy without extra reconstruction, which blocks safe first real-world use. | Partially resolved in docs. [E] | Revalidate target-repo initialization after the documented rewrite requirement is applied in practice. [E] |
